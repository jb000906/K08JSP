...
<head>
    ...
    <script>
        ...
        if(frm.pwd.value != frm.pwd2.value){
            ...
            frm.pwd.value="";//기존에 입력된 
            frm.pwd2.value="";//패스워드를 지워줌
            frm.pwd.focus();
            return false;
        }
        /*
        checkbox와 radio는 기본적으로 value속성값을 가지고 있으므로
        입력값에 대한 검증이 아니라, 선택여부에 대한 검증을 해야함
        */
        var isGender = false;
        //라디오의 갯수만큼 반복함 DOM을 통해 갯수를 알수 있음
        for(var i=0;i<frm.gender.length; i++){
            //체크된 항목이 있는지 확인해서...
            if(frm.gender[i].checked==true){
                //있다면 true로 값 변경 후 루프 탈출
                isGender = true;
                //radio는 하나만 선택할 수 있으므로 더이상 확인 할 이유 없음
                break;
            }
        }
        //성별을 체크하지 않았다면 경고창을 띄워줌
        if(isGender != true){
            alert("성별을 선택해 주세요");
            //라디오는 여러 항목이 있으므로 focus()를 사용할때 인덱스를 써야함
            frm.gender[0].focus();
            return false;
        }
        /*
        ...
        */
        ...
        /*
        textarea는 자체적인 value 속성은 없으나, JS에서 DOM객체를 통한
        ...
        */
        ...
        function idCheck(fn){
            //아이디가 입력되지 않은 상태
            if(fn.id.value==""){
                alert("아이디를 입력후 중복확인을 누르세영");
                fn.id.focus();
            }
            else{
                /*
                중복확인창을 띄울때 기존 입력된 내용을 수정할 수 없도록
                readOnly속성을 추가한다. 만약 수정이 가능하다면 사용자는
                수정할 수도 있으므로 주의가 필요함
                */
                fn.id.readOnly=true;
                /*
                부모창에서 입력된 아이디를 파라미터로 팝업창으로 전달함
